const messages = {
  en: {
    ...
    demoImportDenied: ({ name }) => `Example cannot import undeclared module: ${name}`,
    demoNotAllowed: "This API is not allowed in the server executor",
    invalidDemoIndex: "Example index must be a non-negative integer",
    invalidJson: "Request body must be valid JSON",
    methodNotAllowed: "Method not allowed",
    missingDemoSource: "Missing example source",
    requestTooLarge: "Request exceeds the 100 KB limit",
    serverDemoNotFound: "Server example source was not found",
    serverDemoTimedOut: "Server example timed out",
    sourceTooLarge: "Example source exceeds the 100 KB limit",
    tooManyRequests: "Too many example requests",
  },
  "zh-CN": {
    babelNoOutput: "Babel 未返回编译产物",
    demoImportDenied: ({ name }) => `示例不允许导入未声明的模块：${name}`,
    demoNotAllowed: "该 API 不允许在服务端执行器中运行",
    invalidDemoIndex: "示例索引必须是非负整数",
